...
public class GPUImageFilterTools {  
    public static void showDialog(final Context context,
        ...
        filters.addFilter("Contrast", FilterType.CONTRAST); //对比度
        filters.addFilter("Invert", FilterType.INVERT); //倒置
        filters.addFilter("Hue", FilterType.HUE); //色调
        filters.addFilter("Sepia", FilterType.SEPIA);//4
        filters.addFilter("Grayscale", FilterType.GRAYSCALE);//5
        filters.addFilter("Sharpness", FilterType.SHARPEN);//6
        filters.addFilter("Emboss", FilterType.EMBOSS);//7
        filters.addFilter("Monochrome", FilterType.MONOCHROME); // 8
        filters.addFilter("Vignette", FilterType.VIGNETTE);//9
        filters.addFilter("Sketch", FilterType.SKETCH);//10
        filters.addFilter("Haze", FilterType.HAZE); // 11
...
//        filters.addFilter("Pixelation", FilterType.PIXELATION); // 像素化
        ...
        builder.setItems(filters.names.toArray(new String[filters.names.size()]),
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(final DialogInterface dialog, final int item) {
                        Log.d("filter", filters.names.get(item));
