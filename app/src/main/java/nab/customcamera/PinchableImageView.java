...
public class PinchableImageView extends ImageView {
    ...
    private final static String TAG = "PinchableImageView";
    ...
    public void setRect() {
        ...
        int t = getTop();
        int l = getLeft();
        if(w != 1 && h != 1 && firstLoadBitmap){
            firstLoadBitmap = false;
            // 判断图是横的还竖的  确定最小缩放比 正好宽对齐或者高对齐
            if (bmWidth > bmHeight) {
                minScale = 1.0f * screenW / (bmWidth * scale);
            } else {
                minScale = 1.0f * screenW / (bmHeight * scale);
            }
            Log.d(TAG,minScale + "");
        }
        Log.d(TAG,w + "x" + h);
    ...
    private void onRebound() {
        /* 判断是否超出范围 并处理 */
        int disX = 0, disY = 0;
        if(getHeight() < screenW && getWidth() >= screenW){
            disY = (screenH - getHeight()) / 2 - getTop();
        }else if (getHeight() < (screenW )) {
            disY = ((screenW) - getHeight()) / 2 - getTop();
        }
        if (getWidth() < screenW) {
            disX = (screenW - getWidth()) / 2 - getLeft();
        }
        if (getHeight() >= (screenW)) {
            if (getTop() > cropHeight)
                disY = cropHeight - getTop();
            if (getBottom() < screenH - cropHeight)
                disY = screenH - getBottom() - cropHeight;
        }
        if (getWidth() >= screenW) {
            if (getLeft() > 0)
                disX = -getLeft();
